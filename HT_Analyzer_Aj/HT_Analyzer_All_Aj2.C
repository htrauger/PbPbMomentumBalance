...
int main(int argc, char *argv[]){
  ...
  for(int fi = 0; fi < (int) file_names.size(); fi++) {
	...
	if(is_pp){
	  if(my_primary->HLT_PAJet80_NoJetID_v1==0) {continue; }
	}
	...
	for(int j4i = 0; j4i < (int) my_primary->corrpt->size(); j4i++) {
	  ...
	  if(j4i==highest_idx){
	    ...
	    my_hists[data_mc_type_code]->only_leadingjets_Aj_corrpT[ibin][ibin2][ibin4]->Fill(my_primary->corrpt->at(j4i), wvz*wcen);
	    my_hists[data_mc_type_code]->only_leadingjets_Aj_phi[ibin][ibin2][ibin4]->Fill(my_primary->jtphi->at(j4i), wvz*wcen);
	    my_hists[data_mc_type_code]->only_leadingjets_Aj_eta[ibin][ibin2][ibin4]->Fill(my_primary->jteta->at(j4i), wvz*wcen);	 
	  ...
	  if(j4i==second_highest_idx){
	    ...
	    my_hists[data_mc_type_code]->only_subleadingjets_Aj_corrpT[ibin][ibin2][ibin4]->Fill(my_primary->corrpt->at(j4i),wvz*wcen);
	    my_hists[data_mc_type_code]->only_subleadingjets_Aj_phi[ibin][ibin2][ibin4]->Fill(my_primary->jtphi->at(j4i),wvz*wcen);
	    my_hists[data_mc_type_code]->only_subleadingjets_Aj_eta[ibin][ibin2][ibin4]->Fill(my_primary->jteta->at(j4i),wvz*wcen);
	  ...
	  if(!is_data){
		...
		my_hists[data_mc_type_code]->hJetTrackSignalBackgroundSubLeading_Aj_pTweighted[ibin][ibin2][ibin4][ibin3]->Fill(deta,dphi, track_pt*wvz*wcen);	
	      ...
	      //------------------------------------------------
	      //Repeat, now subdividing into sube==0 and sube>0.   
	      //-----------------------------------------------
	    ...
	    //----------------------------------------------------
	    //      EVENT MIXING STARTS HERE!  (DATA / RECO JETS)
	    //-----------------------------------------------------
	  ...
	  if(do_mixing){
	    ...
	    if(j4i==highest_idx||j4i==second_highest_idx){  //only if we've got a trigger according to some criteria  
		...
		if(is_data){
		  ...
		  for(int tracks =0; tracks < (int) me_tree->pt->size(); tracks++){
		    ...
		    if(!is_pp&&ibin3<3&&me_tree->sube->at(tracks)==0) continue;
		    //---------------------------
		    // Now we are ready to fill!
		    //---------------------------
	...
	for(int j4i = 0; j4i < (int) my_primary->genpt->size(); j4i++) {
	  ...
	  if(j4i==highest_idx_gen){
	    ...
	    my_hists[data_mc_type_code]->only_leadingjets_Aj_corrpT[ibin][ibin2][ibin4]->Fill(my_primary->genpt->at(j4i), wvz*wcen);
	    my_hists[data_mc_type_code]->only_leadingjets_Aj_phi[ibin][ibin2][ibin4]->Fill(my_primary->genphi->at(j4i), wvz*wcen);
	    my_hists[data_mc_type_code]->only_leadingjets_Aj_eta[ibin][ibin2][ibin4]->Fill(my_primary->geneta->at(j4i), wvz*wcen);	 
	  }
	  ...
	  if(j4i==second_highest_idx_gen){
	    ...
	    my_hists[data_mc_type_code]->only_subleadingjets_Aj_corrpT[ibin][ibin2][ibin4]->Fill(my_primary->genpt->at(j4i), wvz*wcen);
	    my_hists[data_mc_type_code]->only_subleadingjets_Aj_phi[ibin][ibin2][ibin4]->Fill(my_primary->genphi->at(j4i), wvz*wcen);
	    my_hists[data_mc_type_code]->only_subleadingjets_Aj_eta[ibin][ibin2][ibin4]->Fill(my_primary->geneta->at(j4i), wvz*wcen);
	  }
	  ...
	  //----------------------------------------------------
	  //      EVENT MIXING STARTS HERE!  (MC GenGen)
	  //-----------------------------------------------------
	  ...
	  if(do_mixing){
		...
		for(int tracks =0; tracks < (int) me_tree->pt->size(); tracks++){
		  ...
		  if(j4i==highest_idx){
		    deta = my_primary->geneta->at(highest_idx) - me_tree->eta->at(tracks);
		    dphi = my_primary->genphi->at(highest_idx) - me_tree->phi->at(tracks);
		    while(dphi>(1.5*TMath::Pi())){dphi+= -2*TMath::Pi();}
		    while(dphi< (-0.5*TMath::Pi())){dphi+= 2*TMath::Pi();}
		  ...
		  if(j4i==second_highest_idx){
		    deta = my_primary->geneta->at(second_highest_idx) - me_tree->eta->at(tracks);
		    dphi = my_primary->genphi->at(second_highest_idx) - me_tree->phi->at(tracks);
		    while(dphi>(1.5*TMath::Pi())){dphi+= -2*TMath::Pi();}
		    while(dphi<(-0.5*TMath::Pi())){dphi+= 2*TMath::Pi();}
